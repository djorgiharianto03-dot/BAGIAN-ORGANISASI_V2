...
if (!function_exists('org_theme_hari_besar_class')) {
    ...
    function org_theme_hari_besar_meta(?DateTimeInterface $date = null): array
    {
        ...
        // Hari keagamaan / periode panjang (mis. 7 hari) — prioritas lebih tinggi
        foreach (org_theme_kalender_list() as $item) {
            $durationDays = (int) ($item['duration_days'] ?? 1);
            if ($durationDays <= 1) {
                continue;
...
/** Label tampilan: underscore → spasi (nama asli di server tidak diubah). */
...
$documentDisplayDescription = static function (string $storedName) use ($displayUploadFilename): string {
    ...
    return $tipe . ' — ringkasan dari nama berkas: "' . $stem . '".';
};
